...
def drawCMSLogo(myC, energy, lumi):
        ...
        CMS_lumi(myC, "106.71 fb^{-1} (13 TeV)", 10)
...
def main():
    ...
    if len(obs) > 0:
        make1DLimitPlot("c#tau [cm]", xvals, obs, exp, exp1plus, exp1minus, exp2plus, exp2minus, theory)
    ...
    if len(obs) > 0:
        make1DLimitPlot("m_{1} [GeV]", xvals, obs, exp, exp1plus, exp1minus, exp2plus, exp2minus, theory)
