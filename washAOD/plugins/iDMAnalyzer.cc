...
void iDMAnalyzer::beginJob()
{
    ...
    recoT->Branch("mu_pt",  &recoPt_);
    recoT->Branch("mu_eta", &recoEta_);
    recoT->Branch("mu_phi", &recoPhi_);
    recoT->Branch("mu_dxy", &recoDxy_);
    ...
    recoT->Branch("PF_jet_eta", &recoPFJetEta_);
    recoT->Branch("PF_jet_phi", &recoPFJetPhi_);
