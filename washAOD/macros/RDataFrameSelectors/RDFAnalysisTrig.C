#include "RDFAnalysisTrig.h"
#include <TH2.h>
#include <TStyle.h>

void RDFAnalysisTrig::SetCuts(std::vector<common::CutInfo> cuts_info) {
    cuts_info_ = cuts_info;
}

void RDFAnalysisTrig::SetParams(common::SampleInfo sample_info, Double_t lumi, TString region = "SR") {
...
Bool_t RDFAnalysisTrig::Process(TChain * chain) {
...
//   };
   ...
   auto calcTotalWgt = [&](float genwgt) {
...
//       Define("wgt", calcTotalWgt, {"Zwgt", "Wwgt", "Twgt", "PUwgt", "gen_wgt"});
       Define("wgt", calcTotalWgt, {"genwgt"});
   }
  ...
  // eff_plot_.clear();
   ...
   if (mode_ != common::DATA) {
       auto df_sumgenwgts = df_wgts.Sum("genwgt");
...
void RDFAnalysisTrig::Terminate() {
   //delete sf_pu;
}
