...
void RDFAnalysis::Begin() {
    ...
    TFile * dsa_wgt_2016 = TFile::Open("../../data/idm_dsa_scalefactors/NUM_DisplacedID_DEN_dSAMuons_abseta_pt_2016.root");
    TFile * dsa_wgt_2017 = TFile::Open("../../data/idm_dsa_scalefactors/NUM_DisplacedID_DEN_dSAMuons_abseta_pt_2017.root");
    TFile * dsa_wgt_2018 = TFile::Open("../../data/idm_dsa_scalefactors/NUM_DisplacedID_DEN_dSAMuons_abseta_pt_2018.root");
...
Bool_t RDFAnalysis::Process(TChain * chain) {
    ...
    */
    ...
    auto dsa_lowpt_lookup = [&](int eta, int err, int year) {
        float sf = 1.0;
        float downerr = 0.;
        float uperr = 0.;
        //if (year == 2018) {
    ...
    auto calcMUsf = [&](RVec<float> muonpt_gm, RVec<float> muoneta_gm, RVec<float> muonpt_dsa, RVec<float> muoneta_dsa, size_t id1, size_t id2) {
          ...
          if( id1 >3) {// global muon
            ...
            if(muonpt_dsa[id1] < 20){ sf2 = dsa_lowpt_lookup(muoneta_dsa[id1],err_type,year_);}
            else{
          ...
          if(id2 > 3) { 
            ...
            if(muonpt_dsa[id2] < 20){ sf2 = dsa_lowpt_lookup(muoneta_dsa[id2],err_type,year_);}
            else{
    ...
    auto calcTotalWgt = [&](float veto_wgt, float muon_wgt, float Zwgt, float Wwgt, float Twgt, float PUwgt, float trig_wgt, float gen_wgt) {
        double weight =  veto_wgt * muon_wgt * Zwgt * Wwgt * PUwgt * trig_wgt * xsec_ * lumi_ * gen_wgt / sum_gen_wgt_;
    ...
    if (mode_ == common::DATA) 
        ...
        df_wgts = df_wgts.
            ...
            Define("muon_wgt", calcMUsf, {"reco_gm_pt", "reco_gm_eta", "reco_dsa_pt", "reco_dsa_eta", "best_muon_0", "best_muon_1"}).
            Define("wgt", calcTotalWgt, {"veto_wgt", "muon_wgt", "Zwgt", "Wwgt", "Twgt", "PUwgt", "trig_wgt", "gen_wgt"});
            //Define("wgt", "1.0");
