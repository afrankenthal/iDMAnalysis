...
import sys
muName = sys.argv[1]
# standAloneMuon, refittedStandAloneMuon,
# standAloneMuonUpdatedAtVertex,
# displacedStandAloneMuon, globalMuon,
# displacedGlobalMuon, cosmicMuon, cosmicMuon1Leg
...
for k, inf in infs.iteritems():
    ...
    for mu in inf.Get('{0}_recoEffi/denom'.format(muName)):
        denom.Fill(mu.pt)
    for mu in inf.Get('{0}_recoEffi/numer'.format(muName)):
        numer.Fill(mu.pt)
...
outfn = '../test/gentrigrecoeffiResults/recoEffi_{0}.pdf'.format(muName)
