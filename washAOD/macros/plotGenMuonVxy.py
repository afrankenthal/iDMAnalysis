...
# ploting gen muons' vertex (vxy) of 
...
for l in sorted(inarrays.keys()):
    ax.hist(inarrays[l]['vxy'], bins=50, range=(0,100), normed=True,
        label=str(l)+' cm', histtype='step')

props = {
    'title': ' gen mu vertex vxy',
    'xlabel': 'vxy [cm]',
    'ylabel': 'A.U.'
}
ax.set(**props)
...
outfn = '../test/gentrigrecoeffiResults/genMuVxy.pdf'
plt.savefig(outfn)