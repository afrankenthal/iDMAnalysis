...
void PlotGenLevelValidation(TString config_filename, bool fSave=0, bool fShow=1) {
    ...
    for (auto const & [obs_cppstr, obs_cfg] : config["observables"].items()) {
        ...
        TH1F * frame = canvases[obs]->DrawFrame(0.9, 0.1, bins[obs][bins[obs].size()-1], 1e5);
