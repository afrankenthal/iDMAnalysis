...
void PlotGenLevelComparisonMCInvMass(TString config_filename, bool fSave=0, bool fShow=1) {
    ...
    for (auto const & [obs_cppstr, obs_cfg] : config["observables"].items()) {
        ...
        TH1F * frame = canvases[obs]->DrawFrame(0, 0, bins[obs][bins[obs].size()-1], 0.1);
